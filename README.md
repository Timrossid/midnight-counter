...
> **Compile artifacts:** The `managed/counter/` directory (compiled circuits,
> proving/verifying keys, ZKIR) is generated by CI on every push and uploaded as
> a downloadable artifact. To inspect it: open the
> [Actions tab](https://github.com/Timrossid/midnight-counter/actions/workflows/ci.yml)
> → click the latest passing run → scroll to **Artifacts** at the bottom →
> download **managed-counter**.
