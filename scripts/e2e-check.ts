/**
 * End-to-end smoke check for midnight-counter.
 *
...
// Must match the privateStateId used at deploy time. The counter's witness
// localSecretKey() reads the owner secret key from this private state.
...
async function main() {
  ...
  // NOTE: findDeployedContract OVERWRITES the stored private state whenever
  // `initialPrivateState` is supplied, so we only supply one when the store is
  // empty. This check is read-only and must never clobber the owner's key.
  try {
    ...
    await findDeployedContract(providers, {
      ...
      ...(existingState ? {} : { initialPrivateState: { secretKey: '0'.repeat(64) } }),
    });
  } catch (err: any) {
