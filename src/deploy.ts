/**
 * Deploy the midnight-counter contract to a Midnight network (undeployed by
...
/** Generate a random 32-byte secret key (64 hex chars) for the counter owner. */
function randomSecretKey(): string {
  return randomBytes(32).toString('hex');
}

// Identifier under which this contract's private state is stored. The counter
// contract's witness localSecretKey() reads its secret key from here, so the
// private state holds that key (32 bytes as 64 hex chars). It never leaves the
// local machine / is never written to the ledger.
...
async function main() {
  console.log('\n╔══════════════════════════════════════════════════════════════╗');
  console.log(`║  Deploy midnight-counter to ${network}`);
  console.log('╚══════════════════════════════════════════════════════════════╝\n');
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      const secretKey = process.env.COUNTER_SECRET_KEY?.trim() || randomSecretKey();
